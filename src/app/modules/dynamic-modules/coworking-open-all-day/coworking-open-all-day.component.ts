...
@Component({
  selector: 'app-coworking-open-all-day',
  templateUrl: './coworking-open-all-day.component.html',
  styleUrls: ['./coworking-open-all-day.component.scss'],
})
export class CoworkingOpenAllDayComponent implements OnInit, OnDestroy {
  loading = true;
  ...
  addSeoTags() {
    this.seoService.getMeta('24-hour-open').subscribe(seoMeta => {
    ...
    this.workSpaceService.getWorkspaces(sanitizeParams(param)).subscribe(allWorkSpaces => {
      ...
      if (allWorkSpaces.data.length) {
        const IMAGE_STATIC_ALT = [
          ...
          '24/7 Coworking Space near me',
          '24 hour Coworking Space Delhi',
          'Coworking 24 hours',
        ];
