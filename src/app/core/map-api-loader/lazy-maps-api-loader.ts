...
/**
 * Token for the config of the LazyMapsAPILoader.
 * Please provide an object of type {@link LazyMapsAPILoaderConfig}.
 */
export const LAZY_MAPS_API_CONFIG = new InjectionToken<LazyMapsAPILoaderConfigLiteral>(
...
export class LazyMapsAPILoader extends MapsAPILoader {
  protected scriptLoadingPromise: Promise<void>;
  protected config: LazyMapsAPILoaderConfigLiteral;
  ...
  load(): Promise<void> {
    const window = this.windowRef.getNativeWindow() as any;
    ...
    const scriptOnPage = this.documentRef.getNativeDocument().getElementById(this._SCRIPT_ID);
    if (scriptOnPage) {
  ...
  private _assignScriptLoadingPromise(scriptElem: HTMLElement) {
    this.scriptLoadingPromise = new Promise<void>((resolve, reject) => {
      (this.windowRef.getNativeWindow() as any)[this.callbackName] = () => {
        resolve();
      };
